/*
 * covert.c — Covert channel tool (Phase 19a)
 *
 ...
 *
 * Usage:
 *   ironattack covert --target <ip> --mode icmp|isn|dns --message <text> [--iface <name>]
 */
...
int cmd_covert(int argc, char **argv) {
    ...
    if (!target || !mode || !message) {
        ...
        fprintf(stderr, "Usage: ironattack covert --target <ip> --mode icmp|isn|dns --message <text> [--iface <name>]\n");
        return 1;
    }

    printf("=== Covert Channel Tool (Phase 19a) ===\n");
    printf("  Target:  %s\n", target);
    printf("  Mode:    %s\n", mode);
    ...
    fprintf(stderr, "Unknown mode: %s (use icmp|isn|dns)\n", mode);
    return 1;
}
